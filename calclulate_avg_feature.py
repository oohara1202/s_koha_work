...
SPEECH_DIR = 'evaluation_data/analysis_vc'
EXP_DIR = 'exp/averaged_feature'
weights_path = '/work/abelab4/s_koha/vits/dump/averaged_vector_studies-teacher/weighted_sum_weights.pt'  # for ssl
...
def main():
    xvector_extractor = ExtractXvector()
    ssl_extractor = ExtractSSLModelFeature()
    # ファイル名をkeyに感情を記録
    ...
    dir_list = [f for f in os.listdir(SPEECH_DIR) if os.path.isdir(os.path.join(SPEECH_DIR, f))]
    for dname in dir_list:
        ...
        wav_dir = os.path.join(SPEECH_DIR, dname)
        wavfiles = sorted(glob.glob(os.path.join(wav_dir, '*.wav')))

        # SSL-model feature
        if feature_name == 'sslfeature':
            ulf = UtteranceLevelFeaturizer()
            for wavfile in wavfiles:
                embeds_ssl = ssl_extractor(wavfile)
        ...
        # Uttrance-level feature
            ...
            for wavfile in wavfiles:
                feature = xvector_extractor(wavfile)
        ...
        save_dir = os.path.join(EXP_DIR, dname, feature_name)
